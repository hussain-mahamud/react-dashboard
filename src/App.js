...
const ServicePlatformAdmin = () => {
  const { t, currentLanguage, isRTL, changeLanguage } = useLocalization();
          ...
          <div className="space-y-4">
            {bookings.slice(0, 5).map(booking => (
              <div key={booking.id} className="flex items-center justify-between py-3 border-b border-gray-100 last:border-b-0">
                ...
                <div className={`text-right ${isRTL ? 'ml-0 mr-4' : 'ml-4'}`}>
            ...
            {services
              ...
              .map(service => (
                <div key={service.id} className="flex items-center justify-between py-2">
                  <div className="flex items-center">
                    <div className={`w-10 h-10 bg-gray-100 rounded-lg flex items-center justify-center ${isRTL ? 'ml-3' : 'mr-3'}`}>
            ...
            <div className={`flex items-center space-x-3 ${isRTL ? 'flex-row-reverse space-x-reverse' : ''}`}>
              <div className="relative">
                ...
                <input
                  ...
                  className={`py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${isRTL ? 'pr-10 pl-4' : 'pl-10 pr-4'}`}
                />
              </div>
            ...
            <thead className="bg-gray-50">
              <tr>
                {columns.map((column, index) => (
                  <th key={index} className={`px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider ${isRTL ? 'text-right' : 'text-left'}`}>
                    {column}
                  </th>
                ))}
                  ...
                  {Object.entries(item).map(([key, value], cellIndex) => (
                    key !== 'id' && (
                      <td key={cellIndex} className="px-6 py-4 whitespace-nowrap">
                        ...
                        ) : key === 'name' && currentLanguage === 'ar' && item.nameAr ? (
                          <span className="text-sm text-gray-900">{item.nameAr}</span>
                        ) : (
    ...
    <div className="bg-white p-6 rounded-lg shadow-sm border">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center">
            <input type="checkbox" id="maintenance" className={isRTL ? 'ml-2' : 'mr-2'} />
  ...
  return (
    <div className={`flex h-screen bg-gray-100 ${isRTL ? 'rtl' : 'ltr'}`} dir={isRTL ? 'rtl' : 'ltr'}>
      {/* Sidebar */}
      <div className={`${sidebarOpen ? 'block' : 'hidden'} lg:block fixed lg:relative inset-y-0 ${isRTL ? 'right-0' : 'left-0'} z-50 w-64 bg-white shadow-lg`}>
        <div className="flex items-center justify-between p-4 border-b">
            ...
            return (
              <button
                ...
                className={`w-full flex items-center px-4 py-3 text-left hover:bg-gray-100 ${
                  activeTab === item.id 
                    ? `bg-blue-50 ${isRTL ? 'border-l-2 border-blue-600' : 'border-r-2 border-blue-600'} text-blue-600` 
                    : 'text-gray-700'
          ...
          <div className="flex items-center justify-between">
            ...
            <div className={`flex items-center space-x-4 ${isRTL ? 'flex-row-reverse space-x-reverse' : ''}`}>
              ...
              <button className="p-2 text-gray-400 hover:text-gray-600 relative">
                ...
                <span className={`absolute -top-1 w-3 h-3 bg-red-500 rounded-full ${isRTL ? '-left-1' : '-right-1'}`}></span>
              </button>
              ...
              <div className="relative">
                ...
                {showProfileMenu && (
                  <div 
                    className={`absolute top-full mt-2 w-48 bg-white rounded-lg shadow-lg border z-50 ${isRTL ? 'left-0' : 'right-0'}`}
                    onMouseEnter={() => setShowProfileMenu(true)}
                    onMouseLeave={() => setShowProfileMenu(false)}
                  >
                    <div className="py-2">
